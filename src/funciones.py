...
def check_objwserp(serpiente: object, objeto: object) -> bool:
    ...
    for (
        ...
        serpiente.cuerpo
    ):  # para que en caso de que se genere una manzana en el cuerpo de la serpiente, se genere otra
...
def fin(
    game_over: bool, win: bool, sonido1: object, sonido2: object, VENTANA: object
) -> None:
    """
...
def draw_cuadricula(
    VENTANA: object,
):  # funcion que crea las lineas verticales y horizontales
    """
    ...
    for i in range(range_x + 1):
        pygame.draw.line(
            VENTANA, (176, 220, 116), (0, i * 10 - 5), (ANCHO, i * 10 - 5)
        )  # lineas horizontales
        for j in range(range_y):
            pygame.draw.line(
                VENTANA, (176, 220, 116), (j * 10 - 5, 0), (j * 10 - 5, ALTO)
            )  # lineas verticales
